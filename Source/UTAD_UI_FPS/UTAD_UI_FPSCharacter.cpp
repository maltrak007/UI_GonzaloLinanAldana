...
void AUTAD_UI_FPSCharacter::BeginPlay()
{
	...
	if (PlayerHUDWidget)
	{
		...
		PlayerHUDInstance->InitializeHUD(this);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("Player HUD Widget not assigned to UTAD_UI_FPSCharacter"));
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow,
		                                 TEXT("Player HUD Widget not assigned to UTAD_UI_FPSCharacter"));
	}

	if(SkillWidget)
	{
		SkillTreeWidgetInstance = CreateWidget<USkillTree>(GetWorld(), SkillWidget);
		SkillTreeWidgetInstance->AddToViewport();
		SkillTreeWidgetInstance->SetVisibility(ESlateVisibility::Hidden);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("Skill Widget not assigned to UTAD_UI_FPSCharacter"));
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow,
										 TEXT("Player HUD Widget not assigned to UTAD_UI_FPSCharacter"));
	}
}
...
void AUTAD_UI_FPSCharacter::Ray()
{
	...
	if (GetWorld()->LineTraceSingleByChannel(*HitResult, StartTrace, EndTrace, ECC_Visibility, *CQP))
	{
		...
		if (HitResult->GetActor()->GetClass()->IsChildOf(AUTAD_UI_FPS_Enemy::StaticClass()))
		{
			bIsHittingEnemy = true;
		}
		else
		{
			bIsHittingEnemy = false;
...
{
	if(SkillTreeWidgetInstance->GetVisibility() == ESlateVisibility::Hidden)
	{
		SkillTreeWidgetInstance->SetVisibility(ESlateVisibility::Visible);
		PlayerHUDInstance->SetVisibility(ESlateVisibility::Hidden);
		UWidgetBlueprintLibrary::SetInputMode_GameAndUIEx(PlayerController,SkillTreeWidgetInstance,EMouseLockMode::LockOnCapture);
		PlayerController->SetShowMouseCursor(true);
	}
	else
	{
		SkillTreeWidgetInstance->SetVisibility(ESlateVisibility::Hidden);
		PlayerHUDInstance->SetVisibility(ESlateVisibility::Visible);
